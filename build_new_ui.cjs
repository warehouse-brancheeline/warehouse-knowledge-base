...
let canvasSheet = parts.canvasSheet;
// Remove any trailing `</div>` that don't belong to canvasSheet.
// The canvasSheet string is from `{/* --- DOCUMENT CANVAS SHEET ...` up to the `) : (`.
// It might have extra closing divs for the containers we removed.
// The original was:
// <div className="flex-1 min-w-0 space-y-4 w-full transition-all ...">
//   {/* inputs */}
//   {/* canvas */}
// </div>
// </div>
// </div>
// So there are exactly 3 closing divs at the end of canvasSheet.
let lastDivs = canvasSheet.lastIndexOf('</div>');
canvasSheet = canvasSheet.substring(0, lastDivs);
lastDivs = canvasSheet.lastIndexOf('</div>');
canvasSheet = canvasSheet.substring(0, lastDivs);
lastDivs = canvasSheet.lastIndexOf('</div>');
canvasSheet = canvasSheet.substring(0, lastDivs).trim();
...
// Fix initial state for activeRibbonTab to be 'Home' instead of 'text'
appCode = appCode.replace(/useState\<'text' \| 'image'\>\('text'\)/, 'useState<string>(\'Home\')');

fs.writeFileSync('src/App.tsx', appCode);
console.log("UI built and injected");
